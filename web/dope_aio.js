...
.dope-btn.primary{background:linear-gradient(135deg,#ff5e3a,#ff2a68);border-color:transparent;color:#fff;font-weight:600}
...
.dope-picker .item.sel,.dope-picker .item:hover{background:#ff5e3a33}
...
.dope-status{font:11px/1.3 system-ui,sans-serif;opacity:.7;padding:2px 4px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;color:var(--input-text,#ccc)}
...
function showHover(anchor, name) {
  ...
  Promise.all([thumbFor(name, true), infoFor(name, true)]).then(([t, info]) => {
    ...
    const kids = [];
    const rating = setting(NSFW_SETTING, "PG13");
    const ceiling = { PG: 1, PG13: 2, R: 4, X: 8, XXX: 16 }[rating] ?? 2;
...
// ------------------------------------------------------------------ lora picker
let pickerEl = null;
async function openPicker(anchor, current, onPick) {
  closePicker();
  injectCSS();
  let loras = [];
  try {
  ...
  const q = $("input", { class: "q", placeholder: `Search ${loras.length} LoRAs…  (↑↓ Enter, Esc)` });
  const list = $("div", { class: "list" });
  const side = $("div", { class: "side" }, $("div", { style: { opacity: 0.6 } }, "Hover a LoRA to preview"));
  const pop = $("div", { class: "dope-pop dope-picker" }, q, $("div", { class: "body" }, list, side));
  pickerEl = pop;
  ...
  const render = () => {
    ...
    const head = $("div", { class: "dope-head" },
      ...
      $("label", { title: "Separate CLIP strength per LoRA", style: { cursor: "pointer", opacity: 0.8 } },
        ...
        " clip"),
      $("span", { class: "dope-btn primary", onclick: (e) => openPicker(e.currentTarget, null, (name) => {
    ...
    const rows = state.loras.map((l, idx) => {
      ...
      const row = $("div", { class: `dope-row${l.on ? "" : " off"}`, draggable: "true" },
        ...
        $("div", { class: "dope-name", title: l.lora || "Pick a LoRA",
          onclick: (e) => openPicker(e.currentTarget, l.lora, (name) => { l.lora = name; render(); emit(); }) },
...
async function openKeyDialog() {
  ...
  const civ = $("input", { type: "password", placeholder: keys.civitai?.set ? `saved (${keys.civitai.redacted}) — type to replace` : "optional — only needed for gated models", autocomplete: "off" });
...
async function grokGenerate(node, button) {
  ...
  try {
    ...
    setWidgetValue(node, "positive", res.positive || "");
    if (v("negative_enabled")) setWidgetValue(node, "negative", res.negative || "");
...
function setStatus(node, text) {
  if (node._dopeStatus) node._dopeStatus.textContent = text;
}
...
app.registerExtension({
  ...
  settings: [
    {
      ...
      type: "combo", options: ["PG", "PG13", "R", "X", "XXX"], defaultValue: "PG13",
  ...
  async beforeRegisterNodeDef(nodeType, nodeData) {
    ...
    nodeType.prototype.onConfigure = function () {
      ...
      requestAnimationFrame(() => updateVisibility(this, false));
      return r;
    };
    ...
    nodeType.prototype.onExecuted = function (out) {
      ...
      if (out?.dope_positive?.[0] != null) setWidgetValue(this, "positive", out.dope_positive[0]);
      if (out?.dope_negative?.[0] != null && W(this, "negative_enabled")?.value) setWidgetValue(this, "negative", out.dope_negative[0]);
      if (out?.dope_status?.[0]) setStatus(this, out.dope_status[0]);
      return r;
    };
